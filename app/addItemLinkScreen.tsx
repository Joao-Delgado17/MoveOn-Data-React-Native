...
const AddItemLinkScreen: React.FC = () => {
  ...
  const loadCurrentValues = async () => {
    ...
    const updatedValues = {
      deploy: tasks["link_deploy"] ?? 0,
      collect: tasks["link_collect"] ?? 0,
      rebalance: tasks["link_rebalance"] ?? 0,
      missing: tasks["link_missing"] ?? 0,
    };

    console.log("📥 Valores reais carregados no AddItemLink:", updatedValues);
  ...
  const handleConfirm = async () => {
    ...
    await Promise.all(
      Object.entries(updatedValues).map(([key, value]) => AsyncStorage.setItem(`link_${key}`, value.toString()))
    );
  ...
  return (
    <LinearGradient colors={["#1A1A1A", "#2A2A2A"]} style={styles.container}>
      <Text style={styles.title}>Tarefas Link</Text>
...
export default AddItemLinkScreen;