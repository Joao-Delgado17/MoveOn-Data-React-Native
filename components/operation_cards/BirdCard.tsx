...
type RootStackParamList = {
  addItemBirdScreen: undefined;
};

const BirdCard: React.FC<{ disabled?: boolean }> = ({ disabled = false }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const values = keys.map(key => tasks[`bird_${key}`] ?? 0);
      ...
      console.log("📥 Dados carregados no BirdCard:", storedValues); // 🔥 Log para depuração
  ...
  // Função para alternar entre expandir e colapsar
  const toggleExpand = () => {
  ...
  return (
    <Animated.View style={[styles.card, { height: animatedHeight }, disabled && styles.disabledCard]}>
          <TouchableOpacity disabled={disabled} onPress={() => !disabled && navigation.navigate("addItemBirdScreen")}>
        <ImageBackground
          source={require("../../assets/images/bird.jpg")}
          style={styles.backgroundImage}
          imageStyle={{ opacity: 0.2, borderRadius: 12 }}
        >
          <LinearGradient colors={["rgba(0, 0, 255, 0.8)", "rgba(0, 0, 255, 0.4)"]} style={styles.overlay}>
